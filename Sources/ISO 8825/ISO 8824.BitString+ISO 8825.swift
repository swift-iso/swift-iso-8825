...
extension ISO_8824.BitString: ISO_8825.DER.ImplicitlyTaggable, ISO_8825.BER.ImplicitlyTaggable {
    ...
    public init(
        ...
        // Decode-side canonicity checks (X.690 §8.6.2.3 and §11.2.1): these run
        // here, ahead of delegating to the ISO_8824 value initializer, so that a
        // wire-decode failure reports the X.690-specific `reason` above rather
        // than the ISO_8824 initializer's own (equivalent but less specific)
        // `ISO_8824.Error` from re-validating the same invariant.
